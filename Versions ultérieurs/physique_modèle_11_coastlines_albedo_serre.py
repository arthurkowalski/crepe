...
def update_plot(time, mois=3):
    ...
    print(f'effet de serre a ajouté en moyenne {round(np.mean(temp_apres-temp))} degrés Celsius')
    ax.clear()
    pt = 0
    for shape in shapes:
    ...
    surf = ax.plot_surface(x, y, z, facecolors=plt.cm.viridis(temp / np.max(temp)), rstride=1, cstride=1, alpha=1, linewidth=1)


    # Mettre à jour les données de la colorbar sans la recréer
...
time_slider = Slider(ax_slider, 'Time (h)', 0, 48, valinit=0, valstep=3)

# Liaison du slider à la fonction de mise à jour
time_slider.on_changed(slider_update)


# Création du colorbar
mappable = plt.cm.ScalarMappable(cmap='viridis')
mappable.set_array(temp-273)
